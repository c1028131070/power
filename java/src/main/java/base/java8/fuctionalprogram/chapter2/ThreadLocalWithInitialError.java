...
public class ThreadLocalWithInitialError {
...
/*    private static ThreadLocal<DateFormat> threadLocal =
    ...
    public static void main(String[] args) {
        DateFormat sdf_date2text = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
         ...
         */
        System.out.println("初始定义时间1： " + sdf_date2text.format(date1));
        System.out.println("初始定义时间2： " + sdf_date2text.format(date2));
        Thread t1 = new Thread(() -> {
            int i = 0;
            while (i<100) {
                String dateString = sdf_date2text.format(date1);
        ...
        Thread t2 = new Thread(() -> {
            int i = 0;
            while (i<100) {
                String dateString = sdf_date2text.format(date2);
